...
const filtersReducer = (state = filtersReducerDefaultState, action) => {
  switch (action.type) {
    case 'SET_TEXT_FILTER': return {...state,text:action.txt};
...
const getVisableExpnses = (expenses, {text, sortBy, startDate, endDate}) => {
...
store.subscribe(()=>{
  ...
  const visableExpenses = getVisableExpnses(state.expenses, state.filters)
  console.log(visableExpenses);
});


const expense1 =  store.dispatch(addexpense({description: 'Rent' , amount: 100, createdAt:1000}));
const expense2 =  store.dispatch(addexpense({description: 'Coffe' , amount: 300, createdAt:-1000}));
...
//console.log({...demoState});//usin spread operator on obj
