...
beforeEach((done) => { // using done to run database.ref('expenses').set(expensesData).then(() => done()); after forEach
const expensesData = {};
...
test('should add expene to database and store', (done) => {
...
test('should add expene with defaults to database and store', (done) => {
  const store = createMockStore({});
  const expenseData = {
    description: '',
    amount: 0,
